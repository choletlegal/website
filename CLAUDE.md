...
/domaines-intervention/permis-amenager/
...
/tribunaux-administratifs/                          Pivot SEO local (hors nav principale, cf. Journal)
/tribunaux-administratifs/besancon/
/tribunaux-administratifs/dijon/
/tribunaux-administratifs/nancy/                    TA + CAA Nancy regroupés
/tribunaux-administratifs/strasbourg/
/contact/
/mentions-legales/
/politique-confidentialite/
```

**Granularité des domaines** : 10 domaines — pas d'éclatement du "Contentieux de l'Urbanisme". Aux 4 domaines urbanisme initiaux et à "Droit de la Fonction Publique" (discipline, avancement, rémunération, accident de service) se sont ajoutés "Permis d'Aménager" (détaché de "PLU et Documents d'Urbanisme", qui le listait en doublon) puis 4 domaines droit public complémentaires : "Droit de l'Environnement et des ICPE", "Domaine Public", "Responsabilité Administrative", "Police Administrative" (cf. Journal). Chaque page a un contenu substantiel propre, pas de page vide.
...
- **Bug corrigé : le filtre Liquid `capitalize` sur les *features* de page domaine mettait en minuscule les acronymes** (`_layouts/domaine.html`) — `capitalize` se comporte comme `String#capitalize` de Ruby, qui met en majuscule le premier caractère et **tout le reste en minuscule**, y compris un acronyme en milieu ou fin de chaîne ("Élaboration et révision de PLU" s'affichait "... de plu", "autorisation d'occupation temporaire (AOT)" aurait affiché "(aot)"). Remplacé par `{{ feature | slice: 0, 1 | upcase }}{{ feature | slice: 1, feature.size }}` : ne touche que le premier caractère, laisse le reste de la chaîne intact — fonctionne aussi bien sur les *features* déjà capitalisées en dur dans `_data/domaines.yml` que sur celles écrites tout en minuscules (`fonction-publique`, domaines droit public ajoutés récemment). Vérifié : build propre, rendu correct sur toutes les pages domaine testées (`PLU`, `AOT` préservés en majuscules, capitalisation de première lettre inchangée partout ailleurs).
- **"Permis d'Aménager" détaché de "PLU et Documents d'Urbanisme"** (10ᵉ domaine) : cette page listait déjà "Permis d'aménager" et "Lotissements" comme *features*, et en évoquait un dans son corps de texte — devenu redondant en en faisant un domaine à part entière (même logique que la séparation initiale permis de construire / déclaration préalable / PLU). Nouvelle page `domaines-intervention/permis-amenager.md`, même gabarit que les autres domaines urbanisme ; entrée ajoutée dans `_data/domaines.yml` juste après "Déclaration Préalable". Nettoyage en contrepartie sur `plu-documents-urbanisme.md` : les deux *features* dupliquées retirées, mention de "permis d'aménager et lotissements" dans le corps de texte remplacée par un lien de maillage interne vers `/domaines-intervention/permis-amenager/`. Page pivot mise à jour ("neuf" → "dix" domaines). Vérifié : build propre, un seul H1, JSON-LD `Service` valide, lien interne fonctionnel, 10 domaines listés sur l'accueil et la page pivot. (Le bug `capitalize` repéré sur cette page en la relisant a été signalé puis corrigé séparément — voir l'entrée juste au-dessus.)
- **Opportunité de performance traitée suite au même rapport PageSpeed Insights** (insight en tête : "Requêtes de blocage du rendu — Économies estimées : 1050 ms", score Performance 92/100) : la CSS Google Fonts était chargée en `<link rel="stylesheet">` direct, donc bloquante, alors que `font-display=swap` (déjà en place) affiche de toute façon la police système de repli entre-temps — la CSS de la police n'est pas nécessaire au premier paint. Basculée sur le motif standard preload + bascule au `onload`, repli `<noscript>` identique à l'ancien comportement si JS désactivé. `tokens.css`/`main.css` volontairement laissés en deux requêtes bloquantes distinctes : mesuré en production (`curl --compressed`) ~6,5 Ko gzippés au total, même origine, HTTP/2 — les fusionner (passage par le convertisseur Sass de Jekyll) apporterait un gain négligeable pour la complexité de build ajoutée. Le point "mise en cache" du même rapport (~9 Kio) n'a pas été traité : `Cache-Control: max-age=600` est fixé par l'infrastructure GitHub Pages/Fastly pour tous les sites hébergés, non personnalisable sans CDN devant le site — hors scope d'un correctif de code, et gain négligeable de toute façon. Vérifié : build propre, `<link rel="preload">` et repli `<noscript>` corrects dans le HTML généré.
- **Deux bugs d'accessibilité corrigés suite à un rapport PageSpeed Insights (mobile) partagé par le porteur du projet** (score Accessibilité 93/100, deux audits en échec ; Performance 92, Bonnes pratiques et SEO 100) :
...
- **Audit externe (outil tiers OpenRush) : site quasi invisible sur les requêtes locales** ("avocat besançon" 390/mois, "besancon avocat" 1600/mois — meilleur concurrent local en position 10-15 seulement, première page pas verrouillée) — ~5 mots-clés indexés, ~3 visites organiques/mois, tout provenant d'un seul article de blog. Deux points distincts investigués :
  - **Faux négatif "pas de sitemap détecté"** : `sitemap.xml`/`robots.txt` vérifiés en direct (HTTP 200, XML valide, `jekyll-sitemap` fonctionne normalement) — le vrai problème est que l'outil d'audit interroge l'apex `cholet-avocat.fr` (sans `www`), qui ne pointe pas vers GitHub Pages mais vers un redirecteur 301 côté registrar DNS vers `https://www.cholet-avocat.fr` ; l'étape de découverte du sitemap de cet outil ne suit pas cette redirection cross-host et bascule en crawl de liens. Sans impact réel sur Google/Bing (sitemap déjà soumis directement via Search Console/Bing Webmaster Tools, indépendamment de toute découverte par crawl) — aucune action de code nécessaire, DNS hors du périmètre de ce repo.
  - **Vrai chantier traité (PR #38)** : rien sur la home ni le `<title>` ne signalait "Besançon" en contenu visible (seule la meta description le mentionnait, en fin de phrase). Corrigé : `title`/`description` de `index.html` réordonnés ("Avocat à Besançon" en position forte), ajout d'une ligne `.eyebrow` ("Avocat à Besançon") dans `header/hero.html` au-dessus des badges thématiques — **H1 et chapô du hero volontairement inchangés** (arbitrage explicite avec le porteur du projet pour préserver la voix de marque du hero, esprit affiche suisse) — nouveau modificateur `.hero-eyebrow` dans `main.css` réutilisant `.eyebrow` déjà posé par `page-header.html` plutôt qu'une classe dupliquée.
  - **Signalé mais non traité (hors scope code)** : la fiche Google Business Profile (établissement) et la cohérence NAP (nom/adresse/téléphone identiques site/GBP/annuaires) pèsent au moins autant que l'on-page pour ces requêtes à fort volume (pack local Google Maps) — à vérifier/créer côté porteur du projet. Vérifié depuis (recherche live sur le nom du cabinet) : Knowledge Graph + avis Google présents, donc fiche existante et fonctionnelle, mais absente du pack local sur les requêtes génériques ("avocat besançon") faute d'avis en nombre suffisant face aux confrères (25 à 130+ avis chacun) — au porteur du projet de solliciter des avis clients (RIN art. 10.2 : sobriété, sans contrepartie, sans filtrage par note, cf. échange avec Claude).
- **Nouvelle section `/tribunaux-administratifs/`** (pivot + Besançon/Dijon/Nancy/Strasbourg) pour capter la longue traîne "spécialité + ville" (ex. "avocat urbanisme Dijon"), suite logique du chantier SEO local ci-dessus. Décisions de conception :
  - **Silo racine transversal aux domaines**, plutôt que niché sous `/domaines-intervention/<domaine>/` : l'option nichée impliquait un produit cartésien 10 domaines × 4 villes (40 pages), dont la quasi-totalité aurait été sans contenu réel vu que seules 7 fiches `_decisions` existent au total. Précédent direct : `/decisions/`, déjà une section racine ajoutée en cours de projet.
  - **Nancy regroupe TA et CAA** plutôt que deux pages séparées (ou une page TA vide) : à Nancy coexistent une juridiction de 1ère instance (Meurthe-et-Moselle, Meuse, Vosges, 0 décision publiée) et la cour d'appel du ressort entier (Besançon, Châlons-en-Champagne, Nancy, Strasbourg — 14 départements, 3 décisions publiées, autant que Besançon). Adresses et ressorts de chaque juridiction vérifiés auprès des sites officiels (justice administrative) avant publication.
  - **Contenu volontairement différencié par page, jamais un texte template recopié avec la ville qui change** : discussion approfondie avec le porteur du projet sur ce point, qui a fait auditer trois cabinets concurrents (giorno-avocat.com, akm-avocat.com, carreras-avocat.fr) utilisant précisément ce pattern — confirmé qu'ils ne mettent que ~15-20 % de contenu réellement local (le reste étant un texte générique), et qu'ils ressortent quand même sur "avocat permis de construire besançon" faute de vraie concurrence sur cette niche. Conclusion retenue : le pattern fonctionne aujourd'hui sur cette niche à faible concurrence mais reste risqué à terme (Google qualifie formellement ce pattern de "doorway pages"/"scaled content abuse", application algorithmique inégale) — d'où le choix de différencier réellement chaque page (Besançon met en avant la proximité géographique réelle avec le cabinet, Nancy explique le lien TA→CAA, Dijon/Strasbourg expliquent l'absence de règle de postulation territoriale devant les juridictions administratives plutôt que d'inventer une preuve d'expérience locale absente) : plus robuste face à un éventuel futur resserrement algorithmique, et plus cohérent avec la déontologie (sincérité de la publicité, RIN).
  - `_data/tribunaux.yml` (nouvelle source unique, même patron que `domaines.yml`), `_layouts/tribunal.html` (repris de `domaine.html`, décisions filtrées par `juridiction` via un tableau plutôt que par `domaine`), `_includes/tribunaux-list.html` (réutilise les classes `.domaines-list`/`.domaine-row` existantes, aucun CSS nouveau). Lien ajouté au footer (Liens rapides) ; pas ajouté à la nav principale, même choix que pour `/decisions/`.
  - Vérifié : build propre, JSON-LD `Service` valide, décisions correctement filtrées (3 sur Besançon, 3 sur Nancy, 0 sur Dijon/Strasbourg sans section vide affichée), 5 nouvelles URLs dans le sitemap.
- **`<title>` de toutes les pages intérieures corrigé pour contenir "Besançon"** : le motif "Page | site.title" de jekyll-seo-tag laissait `site.title` (sans "Besançon") composer le `<title>` de chaque page non-home — vérifié qu'aucune des ~40 pages intérieures (domaines, contact, votre-avocat, blog, décisions, tribunaux) ne portait "Besançon" dans son `<title>`, alors que c'est l'un des signaux SEO locaux les plus déterminants. Corrigé en découplant le `<title>` de `site.title` (qui reste par ailleurs le `name` des blocs JSON-LD `WebSite`/`Attorney`, volontairement non modifié) : `_layouts/base.html` compose désormais le `<title>` manuellement sur toutes les pages avec un suffixe court dédié, `Antonin Cholet — Avocat à Besançon`, placé tout de suite après le " | " pour survivre à la troncature Google même sur les titres de page longs. Vérifié : titres finaux 51-73 caractères sur les pages domaines, JSON-LD `WebSite.name`/`Attorney.name` inchangés, `og:title`/`twitter:title` inchangés (n'utilisaient déjà pas ce suffixe).
- **Champ `geo` (GeoCoordinates) ajouté au schema.org `Attorney`** : `Attorney` hérite de `LocalBusiness`, qui recommande ce champ comme signal structuré standard de local SEO — jusqu'ici absent, alors que `hasMap` contenait déjà les mêmes coordonnées codées en dur dans une URL OpenStreetMap. `author.address.latitude`/`longitude` ajoutés à `_data/author.yml` (mêmes valeurs géocodées Nominatim que l'embed OpenStreetMap de `contact.md`) comme source unique, désormais réutilisée par `geo` et `hasMap` plutôt que les coordonnées dupliquées en dur. Vérifié : build propre, JSON-LD valide sur tout le site, `hasMap` inchangé.
- **Reflow forcé corrigé sur le scroll du header** (défaut mineur signalé par un rapport PageSpeed Insights, score Performance 92/100 mobile) : confirmé et localisé précisément via un audit Lighthouse local (`forced-reflow-insight`, chrome headless — l'API PSI publique étant à quota épuisé) — ~104ms de reflow synchrone imputés à `assets/js/main.js`, sur la ligne du listener `scroll` qui lisait `window.scrollY` pour basculer `.is-scrolled` sur `.site-header`. Cause réelle : le header est en `position: sticky`, et Chrome force un recalcul de mise en page synchrone dès qu'une géométrie de scroll est interrogée après une invalidation de style — un simple `requestAnimationFrame` autour de la même lecture/écriture ne suffit pas à l'éliminer (revérifié : le reflow persistait, seulement déplacé de quelques lignes). Corrigé en remplaçant le listener `scroll` par un `IntersectionObserver` observant une sentinelle invisible (`#scroll-sentinel`, nouveau `<div>` posé juste avant le header dans `_layouts/default.html`, stylée en `position: absolute; height: 40px` dans `main.css` — même seuil que l'ancien `threshold = 40`, aucun impact visuel) : plus aucune lecture de géométrie de scroll depuis le fil principal, le callback ne s'exécute qu'au franchissement du seuil au lieu de tourner à chaque frame de scroll. Vérifié : audit `forced-reflow-insight` passé de 104ms/score 0 à 0 occurrence/score 1 sur un serveur Jekyll local ; comportement fonctionnel revérifié identique via un test headless (Puppeteer) — bascule à 200px de scroll, retour à l'état initial en haut de page, sur la page d'accueil (`hero: true`). Build Jekyll propre. Pas de nouveau passage PageSpeed Insights en production à ce stade (quota API épuisé au moment du correctif) — à reconfirmer par le porteur du projet une fois déployé.
- **Maillage interne entre pages domaines corrigé** (point d'une checklist SEO externe passée en revue) : 9 des 10 pages domaines n'avaient aucun lien sortant vers une autre page domaine dans leur corps de texte (seule `plu-documents-urbanisme` en avait un). Corrigé en liant des mentions déjà présentes dans le texte existant plutôt qu'en ajoutant des phrases artificielles — ex. `contentieux-urbanisme` citait déjà "permis de construire, déclaration préalable, permis d'aménager" en toutes lettres sans les lier. Vérifié : build propre, aucun lien cassé.
- **`priceRange` (schema.org) volontairement non ajouté** : contrairement à un commerce, les honoraires d'avocat varient généralement par dossier — une fourchette de type "€€" laisserait croire à une grille tarifaire fixe qui n'existe pas (point de vigilance de sincérité de la publicité, même logique que les autres points déontologiques déjà rencontrés sur ce projet). Décision explicite du porteur du projet : ne pas l'ajouter pour l'instant plutôt que d'inventer une fourchette ; à revoir si une vraie fourchette honnête (ex. tarif de consultation initiale fixe) devient disponible.
- **`js/main.js` sorti des « Requêtes de blocage du rendu »** (nouveau rapport PageSpeed Insights, toujours 92/100 mobile après le correctif du reflow forcé) : attribut `defer` ajouté sur `<script src="/assets/js/main.js">` (`head-js.html`) — le script était déjà placé juste avant `</body>` donc son exécution ne change pas de moment réel, mais sans `defer`/`async` l'audit Lighthouse `render-blocking-insight` le comptait quand même comme bloquant le rendu. Vérifié : audit `render-blocking-insight` local ne liste plus que `tokens.css`/`main.css` après correctif ; comportement fonctionnel identique via un test headless (Puppeteer) — bascule `.is-scrolled` au scroll et menu burger mobile testés, tous deux OK. Point du même rapport volontairement non retraité (déjà tranché, cf. entrée « Opportunité de performance traitée... » plus haut) : `tokens.css`/`main.css` restent deux requêtes bloquantes distinctes, gain de fusion mesuré négligeable face à la complexité de build ajoutée — confirmé à nouveau ici, `metricSavings` de l'audit à 0ms sur LCP et FCP pour ces deux fichiers. Cache TTL 10 min (GitHub Pages) toujours hors périmètre code, même raison que precedemment. « [Non attribué] 90ms » de reflow forcé dans le rapport de l'utilisateur non reproduit sur deux audits Lighthouse locaux frais contre la prod (mode simulé et mode throttling réaliste, score 0,91 cohérent avec le 92 du rapport) — probable rapport PSI mis en cache avant propagation du déploiement précédent, ou variance normale entre passages ; aucune action possible sans reproduction.
