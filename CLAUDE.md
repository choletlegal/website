...
- **Bug préexistant repéré (non corrigé, hors scope)** : le filtre Liquid `capitalize` utilisé par `_layouts/domaine.html` sur chaque *feature* (`{{ feature | capitalize }}`) se comporte comme `String#capitalize` de Ruby — il met en minuscule **tous** les caractères après le premier, y compris un acronyme au milieu ou en fin de chaîne. Repéré en vérifiant le rendu de `plu-documents-urbanisme/` : "Élaboration et révision de PLU" s'affiche "Élaboration et révision de plu". Affecte potentiellement aussi "autorisation d'occupation temporaire (AOT)" sur `domaine-public/` (déjà mergée, PR #27). Signalé pour correction ultérieure, pas traité ici pour ne pas mélanger avec le sujet de cette PR.
